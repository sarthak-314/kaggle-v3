...
def get_augmix(img_size, aug_params_augmix): 
    def augmix_fn(img, label): 
        if tf.random.uniform(shape=[], minval=0.0, maxval=1.0) < aug_params_augmix['prob']:
            img = augmix(img, img_size, severity=aug_params_augmix['severity'], width=aug_params_augmix['width'], depth=-1) 
        return img, label
    return augmix_fn
